...
1. SwiftUI app shell (`Features/*`, `AppShell`) — native interactive shell
2. DesignSystem Catalog — native component and state validation surface
...
**High-fidelity definition for MVP gate:** screens that exist in wireframes are implemented in SwiftUI with tokens or represented in the native Catalog without dead controls claiming false backend behavior. Prefer honest empty/coming-soon over fake finance/health/mail.

**Synthetic data:** dense weekday schedule with overlapping personal/work events, overdue tasks, one stale proposal, one executable proposal — seeded native mocks (`Mocks/`).
...
|---|---|---|---|---|
| S1 | Maya | Golden path conflict → Approve | SwiftUI shell | Ranking, evidence clarity, approve confidence |
| S2 | Jordan | Offline capture + deferred execute | SwiftUI / offline mock | No data loss; sync labeling |
| S3 | Alex | Location/Weather denied reduced briefing | SwiftUI shell + Settings wire | Degradation honesty |
...
Derived from agent/engineering review of the native shell, wireframes, and journeys (2026-07-15).
...
| F2 | Insights/Memory largely placeholders — golden path must not imply analytics exist | P1 | Track: honest empty states in the native shell |
...
| Finding | Resolution |
|---|---|
| Scope pollution (finance/shopping/health as MVP) | Native app and active docs scrubbed; rules in `.cursor/rules/lifepilot-mvp.mdc` |
...
| F3–F4, F7–F8 | **Open** — tracked below |
...
| F2 | Insights/Memory | No charts without data; native screens match MVP IA |
...
2. Walk golden path on SwiftUI (Simulator/macOS) when toolchain available; otherwise note environment limit and use CI + code review.
3. Complete sessions S1–S5 templates.
4. File or update open findings; P0 blocks “prototype validated” claim.

---

## Acceptance criteria checklist (#32)

- [x] Every MVP screen has a high-fidelity native design path (SwiftUI + tokens/Catalog; placeholders explicitly honest)
